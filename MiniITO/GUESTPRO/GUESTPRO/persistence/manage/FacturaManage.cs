...
namespace GUESTPRO.manage
{
    internal class FacturaManage
    {
        ...
        public bool modificarFacutura(Factura f)
        {
            bool ok = false;

            int resultado = DBBroker.getInstancia().update("update mydb.factura set numfactura = '" + f.numfactura + "', descfactura = '" + f.descfactura + "', importe = " + f.importe + " where idfactura = " + f.idfactura);
        ...
        public Factura getFactura(int numFactura)
        {
            ...
            filas = DBBroker.getInstancia().select("select * from mydb.factura where numFactura = " + numFactura);
