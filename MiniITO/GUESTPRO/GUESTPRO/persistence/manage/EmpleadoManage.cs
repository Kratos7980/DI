...
namespace GUESTPRO.manage
{
    internal class EmpleadoManage
    {
        ...
        public bool modificarEmpleado(Empleado e)
        {
            bool ok = false;

            int resultado = DBBroker.getInstancia().update("update mydb.empleado set nombreemp = '" + e.nombreemp + "', apellidos = '" + e.apellidos + "', csr = " + e.csr + ", idrol = " + e.idrol + " where idempleado = " + e.idempleado);
        ...
        public Empleado last()
        {
            ...
            fila = DBBroker.getInstancia().select("select max(idempleado) from mydb.empleado");
